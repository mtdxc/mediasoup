...
void CopyOnWriteBuffer::SetSize(size_t size) {
  ...
  if (!buffer_.use_count()) {
    buffer_ = std::make_shared<Buffer>(buffer_->data(),
...
void CopyOnWriteBuffer::Clear() {
  ...
  if (buffer_.use_count()) {
    ...
    buffer_ = std::make_shared<Buffer>(0, buffer_->capacity());
  }
  RTC_DCHECK(IsConsistent());
}
...
void CopyOnWriteBuffer::CloneDataIfReferenced(size_t new_capacity) {
  ...
  buffer_ = std::make_shared<Buffer>(buffer_->data(), buffer_->size(),
                                         new_capacity);
  RTC_DCHECK(IsConsistent());
}
